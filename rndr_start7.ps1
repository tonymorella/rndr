...
while ($true) {
  ...
  write-Host "   Donations ETH        - 0x39A6467226E55587D6de0DAB1EB696dAeC3EECAc"
  Write-Host "   Path                 - $mainpath"
  Write-Host "   Last checked         - $date"
  Write-Host "   RNDR Watchdog Uptime - $([math]::Round($TimeSpan.Totalhours,2)) hours"
  Write-Host "   Node Uptime          - $([math]::Round($nodeuptime,2)) hours"
  # Write-Host "   Last Boot Time       - $lastboot"
  Write-Host "   Router               - $router"
  Write-Host "   Jobs Complete        - $rndrjobscompleted"
  Write-Host "   Previews Sent        - $rndrpreviewssent"
  Write-Host "   Thumbnails Sent      - $rndrthumbnailssent"
  Write-Host "   OctaneBench Score    - $rndrscore"
  Write-Host "   ETH Wallet           - $rndrwalletid"
  Write-Host "   RNDR Connection      - $RNDRServerCheck"
